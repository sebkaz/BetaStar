...
logging.basicConfig(filename='struc2vec.log',
		    filemode='w',
			level=logging.DEBUG,
...
def parse_args():
	...
	parser.add_argument('--workers', type=int, default=1,
	                    help='Number of parallel workers. Default is 1 for no random results.')

	parser.add_argument('--seed', default=2023, type=int, 
		     help='Seed for random walk generator. Work only with workers=1')
...
def learn_embeddings():
	...
	model = Word2Vec(walks, vector_size=args.dimensions, 
...
def exec_struc2vec(args):
	...
	G = read_graph()
	G = struc2vec.Graph(G, args.directed, 
		     args.workers, untilLayer = until_layer)
